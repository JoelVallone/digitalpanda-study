...
class ListsSuite extends munit.FunSuite:
  ...
  test("details why one plus one is not three (0pts)") {
    //ssertEquals(1 + 1, 3) // Fix me, please!
  ...
  test("max of a few numbers (10pts)") {
    ...
    assert(max(List(7,1,0)) == 7)
    assert(max(List(1,0,7)) == 7)
    assert(max(List(7)) == 7)
    assert(max(List(1,7,7,0)) == 7)
    assert(max(List(-1,2)) == 2)
  }
  ...
  //import main.concurrent.duration.*
