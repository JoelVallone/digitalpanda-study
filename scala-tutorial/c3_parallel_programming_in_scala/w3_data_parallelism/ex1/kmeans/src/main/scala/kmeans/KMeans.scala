...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val c = points.groupBy(p => findClosest(p, means))
    (for { m <- means } yield m -> ( if (c.contains(m)) c(m) else GenSeq.empty[Point])).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans)
      .map{ case (oldMean: Point, newMean: Point) => oldMean.squareDistance(newMean)}
      .aggregate(true)(
        (c, squareDist) => c && (squareDist <= eta),
        _ && _
      )
  }
